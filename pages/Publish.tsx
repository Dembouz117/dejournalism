      ...
      <Form className = {style.formWrapper}>
        <Row>
            <Form.Group className = "mb-3" controlId = "formTitle">
                ...
                <Form.Control placeholder = "Give a brilliant title for your work" onChange = {addTitleHandler}></Form.Control>
            </Form.Group>
        </Row>
        <Row>
            <Form.Group className = "mb-3" controlId = "formDescription">
                <Form.Label>Description:</Form.Label>
                <Form.Control placeholder = "Tell us a little bit about your work" onChange = {addDescHandler}></Form.Control>
            </Form.Group>
        </Row>
        <Row>
            <Form.Group className = "mb-3" controlId = "formAuthor">
                <Form.Label>Author:</Form.Label>
                <Form.Control placeholder = "Make yourself known" onChange = {addAuthorHandler}></Form.Control>
            </Form.Group>
        </Row>
        <Row>
            <Form.Group className = "mb-3" controlId = "formAuthor">
                <Form.Label>PDF of your article:</Form.Label>
                <Form.Control placeholder = "Make yourself known" type = "file" onChange={handleFileChange}></Form.Control>
            </Form.Group>
        </Row>

        <Button className = {style["add-button"]} variant = "outline-light" onClick = {addInputHandler}>+</Button>
       ...
       {arr.map((obj, idx) => {
                return( <Row  key = {idx} className = "mb-3">
                    <Form.Group as = {Col} className = "mb-3" controlId = {idx.toString()}>
                            {idx === 0 && <Form.Label>Contributor:</Form.Label>}
                            <Form.Control placeholder = "Contributor's name..." onChange = {contributorNameHandler}></Form.Control>
                            {idx === arr.length - 1 && <Form.Text className = "text-muted">Give credit where it's due.</Form.Text>}
                        
                    </Form.Group>
                    <Form.Group as = {Col} className = "mb-3" controlId = {idx.toString()}>
                            {idx === 0 && <Form.Label>Role:</Form.Label>}
                            <Form.Control placeholder = "Contributor's role..." onChange = {contributorRoleHandler}></Form.Control>    
                    </Form.Group>
                    <Form.Group  as = {Col} className = "mb-3" controlId = {idx.toString()}>
                            {idx === 0 && <Form.Label>Percentage of share</Form.Label>}
                            <Form.Control placeholder = "Contributor's share..." type = "number" onChange = {contributorPercentageHandler}></Form.Control>
                        
                    </Form.Group>

                    {idx > 0 && <Button className = "sm-3" variant = "outline-light" className = {style["minus-button"]} onClick = {minusHandler}>-</Button>}
                
                    </Row>)
            })}

            <Button type = "submit" className = {style.submit} variant = "outline-light" onClick = {submitHandler}>Submit</Button>
            </Form>
